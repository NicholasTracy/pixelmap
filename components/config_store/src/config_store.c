...
void pm_config_set_defaults(pm_app_config_t *cfg)
{
    ...
    cfg->ap_enable = true; /* SoftAP on for first-boot wizard; user can keep APSTA */
    cfg->ap_fallback = false;
    cfg->ap_ssid[0] = '\0';
    strncpy(cfg->ap_pass, PM_SETUP_AP_PASS, sizeof(cfg->ap_pass) - 1);
    cfg->web_auth = false;
    cfg->web_pass[0] = '\0';
...
esp_err_t pm_config_load(pm_app_config_t *cfg)
{
    ...
    {
        bool have_setup = nvs_get_i32(h, "setup", &v) == ESP_OK;
        if (have_setup) {
            cfg->setup_complete = v != 0;
        } else {
            /* Migrate pre-wizard builds: treat already-configured devices as complete. */
            cfg->setup_complete = cfg->sta_ssid[0] != '\0' ||
                                  (cfg->web_auth && cfg->web_pass[0] != '\0') ||
                                  (cfg->ap_pass[0] != '\0' && strcmp(cfg->ap_pass, PM_SETUP_AP_PASS) != 0);
        }
    }
...
bool pm_config_ensure_security(pm_app_config_t *cfg)
{
    ...
    if (!cfg->setup_complete) {
        /* Walled SoftAP wizard: known default PSK so users never need USB serial. */
        if (strcmp(cfg->ap_pass, PM_SETUP_AP_PASS) != 0) {
            strncpy(cfg->ap_pass, PM_SETUP_AP_PASS, sizeof(cfg->ap_pass) - 1);
        ...
        cfg->web_auth = false;
        ESP_LOGI(TAG, "Setup wizard pending — join SoftAP (password %s) → http://192.168.4.1/",
                 PM_SETUP_AP_PASS);
        return dirty;
    }

    /* Migrate legacy ui_pin → temporary plaintext for hashing below. */
    ...
    if (cfg->web_pass[0] && !pm_security_is_hashed(cfg->web_pass)) {
        ...
        if (pm_security_hash_password(cfg->web_pass, tmp, sizeof(tmp)) == ESP_OK) {
            ...
            cfg->web_auth = true;
            dirty = true;
            ESP_LOGW(TAG, "Migrated web UI password to salted hash");
        }
    }

    /* Auth only when a password hash is present. */
    if (!cfg->web_pass[0]) cfg->web_auth = false;
    if (cfg->web_auth && !cfg->web_pass[0]) {
        cfg->web_auth = false;
        dirty = true;
    }
...
esp_err_t pm_config_save(const pm_app_config_t *cfg)
{
    ...
    nvs_set_i32(h, "setup", cfg->setup_complete ? 1 : 0);
